  ...
  def self.getScaledLevel
    ...
    for i in 0...$PokemonStorage.maxBoxes()
      for j in 0...$PokemonStorage.maxPokemon(i)
        ...
        if(pkmn != nil)
          for k in 0... pc.length()
            if(pkmn.level() > highest[k])
              highest[k] = pkmn.level()
              pc[k] = pkmn
              break
            end #if
          end #for k
    ...
    inparty = $player.party.dup
    pcANDparty = inparty + pc.compact
    level = pbBalancedLevel(pcANDparty) - 2 # pbBalancedLevel increses level by 2 to challenge the player
